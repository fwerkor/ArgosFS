...
directory baseline, ArgosFS loop-backed FUSE `mount-root` using the default
bounded group-commit policy when `/dev/fuse` is usable, and an ext4 loop
baseline when the runner permits loop mounts. Strict, deferred-journal, and
batched-metadata scenarios remain selectable with `--scenarios` for targeted
